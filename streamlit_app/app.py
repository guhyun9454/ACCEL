...
CURVE_DEFS = {
    ...
    "ours_same": {
        "x_key": "p",
        "label": "Ours (Latin Gaussian, th2=th1)",
...
def _curve_series_from_payload(
    ...
    ours_pride_variant: legacy compatibility hook. Defaults to 'th1/2'.
    """
    if not isinstance(payload, dict):
        return {}
    ...
    if curve_key == "ours_same":
        curve = curves.get("swap_gaussian_same", {}) or {}
        ...
        if by_alpha:
            ...
            if alpha_key and alpha_key in by_alpha:
                ...
                if isinstance(alpha_curves, dict):
                    variant = ours_pride_variant or "th1/2"
                    if variant in alpha_curves:
                        curve = alpha_curves[variant]
                    else:
                        curve = alpha_curves.get("th1/2") or alpha_curves
                else:
                    curve = alpha_curves
...
curve_keys = st.multiselect(
    ...
    default=["cyclic", "default_pride", "ours", "ours_sqrt", "ours_same", "ours_pride"],
...
with col_p4:
    max_pct_ours_sqrt = st.selectbox(
        "Ours sqrt 상한",
        options=pride_ours_options,
        index=len(pride_ours_options) - 1,
        format_func=lambda x: f"{x}%" if x < 100 else "100% (전체)",
...
with col_p5:
    max_pct_ours_same = st.selectbox(
        "Ours same 상한",
        options=pride_ours_options,
        index=len(pride_ours_options) - 1,
        format_func=lambda x: f"{x}%" if x < 100 else "100% (전체)",
...
with col_m4:
    min_pct_ours_sqrt = st.selectbox(
        "Ours sqrt 하한",
        options=pride_ours_options,
        index=0,
        format_func=lambda x: f"{x}%",
...
with col_m5:
    min_pct_ours_same = st.selectbox(
        "Ours same 하한",
        options=pride_ours_options,
        index=0,
        format_func=lambda x: f"{x}%",
...
max_pct_by_curve = {
    ...
    "ours_same": float(max_pct_ours_same),
...
min_pct_by_curve = {
    ...
    "ours_same": float(min_pct_ours_same),
...
with col_f:
    lab_ours_same = st.text_input("OURS same 라벨", value=CURVE_DEFS["ours_same"]["label"])
with col_g:
