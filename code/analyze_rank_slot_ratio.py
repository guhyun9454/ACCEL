#!/usr/bin/env python3
"""
Rank-slot probability-ratio analysis.
...
For each rank r and displayed slot s, we collect:
    ...
    ratio(r, s) = p_view(content_(r)) / p_ref(content_(r))
...
def _analyze_results_file(results: List[dict], eps: float = 1e-12) -> Tuple[Optional[int], Dict[str, Dict[str, List[float]]]]:
    ...
    for r in results:
        ...
        for rank_idx, content_idx in enumerate(rank_order, start=1):
            ...
            denom = max(float(ref_prob), float(eps))
            for view_idx, perm in enumerate(selected_perm_tuples):
                ...
                ratio = float(view_prob / denom)
                bucket[f"rank{int(rank_idx)}"][slot_label].append(ratio)
...
def _run_multi(
    *,
    ...
    output: Dict[str, object] = {"results_dirs": results_dirs, "ratio_eps": float(eps), "by_k": {}}
    print("==== Rank-Slot Ratio Analysis ====")
    for k in sorted(combined_by_k.keys()):
        ...
        if save_plots:
            grid_path = os.path.join(aggregate_out_dir, f"multi_model_rank_slot_ratio_grid_k{int(k)}.png")
            mean_path = os.path.join(aggregate_out_dir, f"multi_model_rank_slot_ratio_mean_k{int(k)}.png")
            std_path = os.path.join(aggregate_out_dir, f"multi_model_rank_slot_ratio_std_k{int(k)}.png")
            saved_grid = _save_rank_slot_grid_plot(
                summary=summary,
                bucket=bucket,
                out_path=grid_path,
                title=f"Multi-model cyclic rank-slot ratio distributions (k={int(k)})",
            )
            saved_mean = _save_rank_slot_heatmap(
                summary=summary,
                out_path=mean_path,
                value_key="mean",
                title=f"Multi-model cyclic rank-slot ratio mean (k={int(k)})",
            )
            saved_std = _save_rank_slot_heatmap(
                summary=summary,
                out_path=std_path,
                value_key="std",
                title=f"Multi-model cyclic rank-slot ratio std (k={int(k)})",
            )
            for p in (saved_grid, saved_mean, saved_std):
                if p:
                    print(f"Saved: {p}")

    out_path = os.path.join(aggregate_out_dir, "multi_model_rank_slot_ratio_summary.json")
...
def _run_single(
    *,
    ...
    output: Dict[str, object] = {"cache_dir": cache_dir, "ratio_eps": float(eps), "by_k": {}}
    print("==== Rank-Slot Ratio Analysis ====")
    ...
    for k in sorted(combined_by_k.keys()):
        ...
        if save_plots:
            grid_path = os.path.join(cache_dir, f"rank_slot_ratio_grid_k{int(k)}.png")
            mean_path = os.path.join(cache_dir, f"rank_slot_ratio_mean_k{int(k)}.png")
            std_path = os.path.join(cache_dir, f"rank_slot_ratio_std_k{int(k)}.png")
            saved_grid = _save_rank_slot_grid_plot(
                summary=summary,
                bucket=bucket,
                out_path=grid_path,
                title=f"Cyclic rank-slot ratio distributions (k={int(k)})",
            )
            saved_mean = _save_rank_slot_heatmap(
                summary=summary,
                out_path=mean_path,
                value_key="mean",
                title=f"Cyclic rank-slot ratio mean (k={int(k)})",
            )
            saved_std = _save_rank_slot_heatmap(
                summary=summary,
                out_path=std_path,
                value_key="std",
                title=f"Cyclic rank-slot ratio std (k={int(k)})",
            )
            for p in (saved_grid, saved_mean, saved_std):
                if p:
                    print(f"Saved: {p}")

    out_path = os.path.join(cache_dir, "rank_slot_ratio_summary.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"Saved: {out_path}")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze cyclic-ensemble rank-slot probability ratios.")
    ...
    ap.add_argument("--ratio_eps", type=float, default=1e-12, help="Stability epsilon for p_view / max(p_ref, eps).")
