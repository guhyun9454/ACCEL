...
For each rank r and displayed slot s, we collect:
    ...
    logratio(r, s) = log((p_view(content_(r)) + eps) / (p_ref(content_(r)) + eps))
...
def _analyze_results_file(results: List[dict], eps: float = 1e-12) -> Tuple[Optional[int], Dict[str, Dict[str, List[float]]]]:
    if not results:
        return None, {}
    ...
    for r in results:
        ...
        for rank_idx, content_idx in enumerate(rank_order, start=1):
            ...
            for view_idx, perm in enumerate(selected_perm_tuples):
                ...
                logratio = float(np.log((view_prob + float(eps)) / (ref_prob + float(eps))))
                bucket[f"rank{int(rank_idx)}"][slot_label].append(logratio)
...
def _save_rank_slot_grid_plot(
    *,
    ...
    fig, axes = plt.subplots(len(rank_keys), len(labels), figsize=(3.2 * len(labels), 2.6 * len(rank_keys)), dpi=160, squeeze=False)
    ...
    *,
    ...
    for results_dir in results_dirs:
        ...
        for ci in cache_files:
            ...
            k, bucket = _analyze_results_file(results, eps=float(eps))
            if k is None:
                continue
    ...
    output: Dict[str, object] = {"results_dirs": results_dirs, "logratio_eps": float(eps), "by_k": {}}
    print("==== Rank-Slot Log-Ratio Analysis ====")
    for k in sorted(combined_by_k.keys()):
        ...
        if save_plots:
            grid_path = os.path.join(aggregate_out_dir, f"multi_model_rank_slot_logratio_grid_k{int(k)}.png")
            mean_path = os.path.join(aggregate_out_dir, f"multi_model_rank_slot_logratio_mean_k{int(k)}.png")
            std_path = os.path.join(aggregate_out_dir, f"multi_model_rank_slot_logratio_std_k{int(k)}.png")
            saved_grid = _save_rank_slot_grid_plot(
                summary=summary,
                bucket=bucket,
                out_path=grid_path,
                title=f"Multi-model cyclic rank-slot log-ratio distributions (k={int(k)})",
            )
            saved_mean = _save_rank_slot_heatmap(
                summary=summary,
                out_path=mean_path,
                value_key="mean",
                title=f"Multi-model cyclic rank-slot log-ratio mean (k={int(k)})",
            )
            saved_std = _save_rank_slot_heatmap(
                summary=summary,
                out_path=std_path,
                value_key="std",
                title=f"Multi-model cyclic rank-slot log-ratio std (k={int(k)})",
            )
            for p in (saved_grid, saved_mean, saved_std):
                if p:
                    print(f"Saved: {p}")

    out_path = os.path.join(aggregate_out_dir, "multi_model_rank_slot_logratio_summary.json")
...
def _run_single(
    *,
    ...
    output: Dict[str, object] = {"cache_dir": cache_dir, "logratio_eps": float(eps), "by_k": {}}
    print("==== Rank-Slot Log-Ratio Analysis ====")
    ...
    for k in sorted(combined_by_k.keys()):
        ...
        if save_plots:
            grid_path = os.path.join(cache_dir, f"rank_slot_logratio_grid_k{int(k)}.png")
            mean_path = os.path.join(cache_dir, f"rank_slot_logratio_mean_k{int(k)}.png")
            std_path = os.path.join(cache_dir, f"rank_slot_logratio_std_k{int(k)}.png")
            saved_grid = _save_rank_slot_grid_plot(
                summary=summary,
                bucket=bucket,
                out_path=grid_path,
                title=f"Cyclic rank-slot log-ratio distributions (k={int(k)})",
            )
            saved_mean = _save_rank_slot_heatmap(
                summary=summary,
                out_path=mean_path,
                value_key="mean",
                title=f"Cyclic rank-slot log-ratio mean (k={int(k)})",
            )
            saved_std = _save_rank_slot_heatmap(
                summary=summary,
                out_path=std_path,
                value_key="std",
                title=f"Cyclic rank-slot log-ratio std (k={int(k)})",
            )
            for p in (saved_grid, saved_mean, saved_std):
                if p:
                    print(f"Saved: {p}")

    out_path = os.path.join(cache_dir, "rank_slot_logratio_summary.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"Saved: {out_path}")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze cyclic-ensemble rank-slot log-ratios.")
    ...
    ap.add_argument("--logratio_eps", type=float, default=1e-12, help="Stability epsilon for log((p_view+eps)/(p_ref+eps)).")
