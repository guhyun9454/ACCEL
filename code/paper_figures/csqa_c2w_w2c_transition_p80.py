...
# ---------------------------------------------------------
# Font and Style Settings (Regular weight, All black)
# ---------------------------------------------------------
...
# Low W2C / Low C2W rates at p=80 (within low subset), per transition step
transitions = ["1->2", "2->3", "3->4", "4->5"]
w2c_rates = np.array([0.0679, 0.0310, 0.0263, 0.0203], dtype=float)
c2w_rates = np.array([0.0437, 0.0302, 0.0209, 0.0183], dtype=float)
...
# ---------------------------------------------------------
# Mountain plot (left=C2W, right=W2C; center=1->2)
# ---------------------------------------------------------
n = len(transitions)
xs_left = list(range(-(n - 1), 1))          # -(n-1) ... 0
xs_right = list(range(0, n))               # 0 ... (n-1)

# left wants reversed order: last transition on far left
...
xticks = xs_left[:-1] + xs_right  # avoid duplicating 0
