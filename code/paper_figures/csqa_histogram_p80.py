...
# ---------------------------------------------------------
# CSQA p=80 settings (from csqa_paper_extract (1).md)
# ---------------------------------------------------------
N_TOTAL = 1216
P = 80
...
low_ratio = 0.7993
low_acc_stage1 = 0.6703

# Per-transition low-subset flip rates at p=80 (within low subset)
# 1->2, 2->3, 3->4, 4->5
w2c_step_rates = np.array([0.0, 0.0679, 0.0310, 0.0263, 0.0203], dtype=float)
c2w_step_rates = np.array([0.0, 0.0437, 0.0302, 0.0209, 0.0183], dtype=float)
...
# left bars want cumulative vs original Stage-1 base
w2c_counts = np.cumsum(gained_correct_counts)
c2w_counts = np.cumsum(gained_wrong_counts)

stages = ["Stage 1\n(Initial)", "Stage 1 -> 2", "Stage 1 -> 3", "Stage 1 -> 4", "Stage 1 -> 5"]
...
print(f"[CSQA p={P}] N_TOTAL={N_TOTAL}, low_ratio={low_ratio}, N_LOW≈{N_LOW}")
...
print("Cumulative flips vs Stage-1 base:")
print("  W2C_cum:", w2c_counts.tolist())
print("  C2W_cum:", c2w_counts.tolist())
...
if not use_broken_axis:
    ...
    # --- Count annotations (same semantics as the broken-axis version) ---
    ...
    for i in range(len(stages)):
        if i == 0:
            ...
            # Left bars (fixed base breakdown)
...
for ax in [ax1, ax2, ax3]:
    # 1) Left bars: fixed base pool breakdown (cumulative vs Stage-1)
    ax.bar(x - offset, c2c_counts, width, color=color_c2c, edgecolor="black", zorder=3)
    ax.bar(x - offset, c2w_counts, width, bottom=c2c_counts, color=color_lost,
           hatch="//", edgecolor="black", zorder=3)
    ...
    # 2) Right bars: gained additions (incremental per step)
    ax.bar(x + offset, gained_correct_counts, width, color=color_gained_correct,
           edgecolor="black", zorder=3)
    ax.bar(x + offset, -gained_wrong_counts, width, color=color_gained_wrong,
           edgecolor="black", zorder=3)
...
# ---------------------------------------------------------
# Set Y-limits (auto-ish, based on base magnitudes)
# ---------------------------------------------------------
mid_lim = max(
    300,
