...
def _plot_three_curves_acc_recall_std(
    ...
    cost_ours_theory, acc_ours_theory, rstd_ours_theory, acc_std_ours_theory, rstd_std_ours_theory = _agg_heur(derived_records_by_p, THEORY_OURS_LABEL, pride_ours_fractions)
    ...
    if derived_records_pride_by_alpha:
        ...
        cost_ours_theory_pride, acc_ours_theory_pride, rstd_ours_theory_pride, acc_std_ours_theory_pride, rstd_std_ours_theory_pride = _agg_heur_by_th1_p(cobjs_op, pride_ours_fractions, THEORY_OURS_LABEL) if cobjs_op else _def5
        ...
        cost_ours_theory_pride, acc_ours_theory_pride, rstd_ours_theory_pride, acc_std_ours_theory_pride, rstd_std_ours_theory_pride = _agg_heur(derived_records_pride_by_p, THEORY_OURS_LABEL, pride_ours_fractions) if derived_records_pride_by_p else _def5
        ...
        if by_alpha:
            ...
            for alpha in prefix_list:
                ...
                if not cobjs:
                    continue
                entry = {}
                for variant in ("th1/2", THEORY_OURS_LABEL, PRIMARY_OURS_LABEL, "online_sqrt_all", TOP1LAST_OURS_LABEL):
    ...
    try:
        ...
        payload = {
            ...
            "curves": {
                ...
                "ours_theory": {
                    "label": THEORY_OURS_LABEL,
                    "p": [float(x) for x in pride_ours_fractions],
                    "cost": [float(x) if np.isfinite(x) else float("nan") for x in cost_ours_theory],
                    "acc": [float(x) if np.isfinite(x) else float("nan") for x in acc_ours_theory],
                    "recall_std": [float(x) if np.isfinite(x) else float("nan") for x in rstd_ours_theory],
                    "delta_acc": [float(a - default_acc) if np.isfinite(a) and np.isfinite(default_acc) else float("nan") for a in acc_ours_theory],
                    "delta_recall_std": [float(default_recall_std - r) if np.isfinite(r) and np.isfinite(default_recall_std) else float("nan") for r in rstd_ours_theory],
                    "delta_acc_std": [float(x) if np.isfinite(x) else 0.0 for x in acc_std_ours_theory],
                    "delta_recall_std_std": [float(x) if np.isfinite(x) else 0.0 for x in rstd_std_ours_theory],
                },
                "ours_top1last": {
