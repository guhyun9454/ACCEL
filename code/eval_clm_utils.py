...
def parse_arguments():
    ...
    parser.add_argument("--empirical_transition_mode", type=str, default="latin",
                        choices=["latin", "latin_swaponly", "latin_swaponly_random", "latin_swaponly_worst", "probe_cyclic", "cyclic_random", "cyclic_targeted", "cyclic_learned"],
                        help="Empirical PriDe transition path. latin keeps stage-by-stage Latin expansion; latin_swaponly is an ablation of latin whose stage-2 swaps ONLY the top1/runner-up slots and freezes the remaining options (disables the shift of the rest), completing later stages best-effort; latin_swaponly_random is the targeting control that swaps top1 with a random other option (rest frozen); latin_swaponly_worst swaps top1 with the lowest-ranked option (rest frozen); cyclic_random incrementally adds random cyclic permutations; cyclic_targeted starts with the cyclic shift that moves the runner-up into the top-1 slot, then adds the remaining cyclic shifts; cyclic_learned chooses a top-3 relative cyclic action sequence from the alpha-prefix validation subset by weighted NLL gain; probe_cyclic is the legacy base -> probe -> final fallback path.")
